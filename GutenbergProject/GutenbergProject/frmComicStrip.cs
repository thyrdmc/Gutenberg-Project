...
{
    public partial class frmComicStrip : Form
    {
        Classes.Connection connection = new Classes.Connection();
        DataSet ds = new DataSet();

        public frmComicStrip()
        {
            InitializeComponent();
        }
        ...
        private void ShowData()
        {
            ...
            conn.Open();
            SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT BookName, AuthorName, Category, Page, DateOfIssue, Book FROM products  WHERE Category = 'Çizgi Roman'", conn);
            dataAdapter.Fill(ds, "products");
            dataGridView1.DataSource = ds.Tables["products"];
            conn.Close();
        }

        private void frmComicStrip_Load(object sender, EventArgs e)
        {
            ShowData();
        }
