...
{
    public partial class frmPersonalGrowth : Form
    {
        Classes.Connection connection = new Classes.Connection();
        DataSet ds = new DataSet();

        public frmPersonalGrowth()
        {
            InitializeComponent();
        }
        ...
        private void ShowData()
        {
            ...
            conn.Open();
            SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT BookName, AuthorName, Category, Page, DateOfIssue, Book FROM products  WHERE Category = 'Kişisel Gelişim'", conn);
            dataAdapter.Fill(ds, "products");
            dataGridView1.DataSource = ds.Tables["products"];
            conn.Close();
        }

        private void frmPersonalGrowth_Load(object sender, EventArgs e)
        {
            ShowData();
        }
