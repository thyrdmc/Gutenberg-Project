...
{
    public partial class frmScienceFiction : Form
    {
        Classes.Connection connection = new Classes.Connection();
        DataSet ds = new DataSet();

        public frmScienceFiction()
        {
            InitializeComponent();
        }
        ...
        private void ShowData()
        {
            ...
            conn.Open();
            SqlDataAdapter dataAdapter = new SqlDataAdapter("SELECT BookName, AuthorName, Category, Page, DateOfIssue, Book FROM products  WHERE Category = 'Bilim Kurgu'", conn);
            dataAdapter.Fill(ds, "products");
            dataGridView1.DataSource = ds.Tables["products"];
            conn.Close();
        }

        private void frmScienceFiction_Load(object sender, EventArgs e)
        {
            ShowData();
        }
